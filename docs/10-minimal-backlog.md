...
#### Story 2.1: 複数履修科目選択
**As a** 学生
**I want** 履修したい複数の科目を一括で選択する
**So that** 効率的に履修登録に向けて科目を準備できる

**受け入れ条件**:
```gherkin
Scenario: 履修選択状況の記録
Given 学生ID "S12345678" がシステムに存在する
And 科目ID "C123456", "C234567", "C345678" が提供されている
...
Scenario: 単位数制限超過時のエラー
Given 学生ID "S12345678" がシステムに存在する
And 科目ID "C123456" (4単位), "C234567" (4単位), "C345678" (4単位) が提供されている
And 学生の単位数制限が10単位に設定されている
And 学期ID "2024-Spring" が設定されている
When 学生が複数科目（合計12単位）を選択するコマンドを実行する
...
- SelectCourses コマンドの実装（複数科目対応）
- CoursesSelected イベントの実装
- ドメインエラーの実装（重複選択、単位上限の基本チェック）
- アプリケーション層のコマンドハンドラー実装
- StudentRegistration 集約の基本実装（複数科目対応）
...
**技術的受け入れ条件**:
- SubmitRegistration コマンドの実装（複数科目対応）
- RegistrationSubmitted イベントの実装
- 基本的な検証のみ（複数科目の単位数集計、最小実装）
...
When 学生が複数科目を選択し、登録を提出し、教務職員が承認し、教員が授業を開始する
Then 全ての工程が正常に完了する
And 各段階で適切なイベントが発生する（CoursesSelected等）
And 最終的に学生が複数の授業に参加可能な状態になる
```
...
- **機能指標**
  - 複数科目履修選択から授業開始まで5分以内で完了
  - 複数科目同時履修者100名でのパフォーマンス維持
  - 基本的なエラーハンドリング動作（重複選択、単位上限など）
